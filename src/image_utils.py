...
class Image:
    ...
    def check_image_limits(self):
        ...
        xx = np.logical_and.reduce(limits)
        self.spherical_proj = self.spherical_proj[:, xx]
        if self.eqr_coord is not None:
            self.eqr_coord = self.eqr_coord[:, xx]
        if self.points_values is not None and len(self.points_values) == len(xx):
            self.points_values = self.points_values[xx]
